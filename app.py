...
selection_mode = st.sidebar.radio(
    "Select feature by:", ("All Features", "Features in Paper")
)
...
if selection_mode == "All Features":
    ...
    selected_paper_feature = st.sidebar.selectbox(
        "Select Feature from Paper",
        list(paper_features.items()),
        format_func=lambda x: f"Feature {x[0]} (Score: {json.load(open(f'autointerp_features/feature_{x[0]}.json'))['score']})",
    )
